...
def insertV1(L, i):
    ...
    position = i
    while not isSorted(L[0:position+1]):
        i = i - 1
        swap(L,i,i-1)      
...
def letterCounts(fname):
    ...
    file = open(fname,'r')
    dct = {'non-letter':0,'a':0,'b':0,'c':0,'d':0,'e':0,'f':0,'g':0,
           'h':0,'i':0,'j':0,'k':0,'l':0,'m':0,'n':0,'o':0,'p':0,'q':0,
           'r':0,'s':0,'t':0,'u':0,'v':0,'w':0,'x':0,'y':0,'z':0}
    newList = []
    for line in file:
        for x in line:
            dct[letter(x)] = dct[letter(x)] + 1

    for item in dct:
        if not (dct[item] == 0):
            newList.append((dct[item],item))
